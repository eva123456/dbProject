...
def listThreadsf(request):
    ...
    if(since_id):
        since_id = since_id[0]
        since_query = ' AND Thread.date >= ' + set_quots(since_id)
    if(limit):
        limit = limit[0]
        limit_query = ' LIMIT {}'.format(limit)
    order = order[0]
    ...
    query = 'SELECT idThread FROM Thread WHERE forum = ' + set_quots(cursor.fetchone()[0]) + full_opt_query +';'
...
def listp(request):
    ...
    if ('forum' in params.keys()):
        ...
        if cursor.rowcount == 0:
            return JsonResponse({'code' : 0, 'response' : response})
        query1 = 'SELECT idPost FROM Post WHERE forum = ' +  set_quots(cursor.fetchone()[0])

    if('thread' in params.keys()):
        sub_query = 'WHERE thread = {}'.format(required[0])
        query1 = 'SELECT idPost FROM Post ' + sub_query
    ...
    if(since_id):
        since_id = since_id[0]
        since_query = ' AND Post.date >= ' + set_quots(since_id)
    if(limit):
        limit = limit[0]
        limit_query = ' LIMIT {}'.format(limit)
    order = order[0]
    order_query = ' ORDER BY Post.date {}'.format(order)
    full_opt_query = since_query + order_query + limit_query

    query = query1 + full_opt_query +';'
...
def listpostsu(request):
    ...
    order_query = ' ORDER BY Post.date {}'.format(order)
    full_opt_query = since_query + order_query + limit_query
    sub_query = ' User.email = ' + set_quots(required[0]) + ' '
    query = 'SELECT Post.idPost FROM Post JOIN User on Post.user = User.idUser WHERE' + sub_query + full_opt_query +';'
...
def listt(request):
    ...
    if ('forum' in params.keys()):
        ...
        if cursor.rowcount == 0:
            return JsonResponse({'code' : 0, 'response' : response})
        query1 = 'SELECT idThread FROM Thread WHERE forum = ' +  set_quots(cursor.fetchone()[0])

    if('user' in params.keys()):
        sub_query = 'SELECT idUser FROM User WHERE email = ' + set_quots(required[0]) + ';'
        cursor.execute(sub_query)
        db.commit()
        if cursor.rowcount == 0:
            return JsonResponse({'code' : 0, 'response' : response})
        query1 = 'SELECT idThread FROM Thread WHERE user = ' +  set_quots(cursor.fetchone()[0])

    #if ('forum' in params.keys()):
        #sub_query = 'Forum on Thread.user = Forum.idForum WHERE Forum.short_name = ' + set_quots(required[0]) + ' '
    #if('user' in params.keys()):
        #sub_query ='User on Thread.user = User.idUser WHERE User.email = ' + set_quots(required[0]) + ' '
    ...
    full_opt_query = since_query + order_query + limit_query
    query = query1 + full_opt_query + ';'
    #query = 'SELECT Thread.idThread FROM Thread JOIN ' + sub_query + full_opt_query +';'
    #cursor = db.cursor()
    cursor.execute(query)
    db.commit()
    #response = []
...
def get_details(table, key, value):
    response = {}
    columns = {'User'   : ['idUser', 'about', 'email', 'isAnonymous', 'name', 'username'], 
               'Forum'  : ['idForum', 'name', 'short_name', 'user'], 
